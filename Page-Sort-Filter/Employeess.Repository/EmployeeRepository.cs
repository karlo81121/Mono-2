...
namespace Employeess.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<List<Employee>> GetAllEmployeesAsync(Paging paging, Sorting sorting, EmployeeFiltering filtering)
        {
            ...
            if (filtering.MinDateOfBirth != null && filtering.MaxDateOfBirth != null)
            {
                stringBuilder.Append(" WHERE birth_date BETWEEN @MinDateOfBirth AND @MaxDateOfBirth ");
                sqlCommand.Parameters.AddWithValue("@MinDateOfBirth", filtering.MinDateOfBirth);
                sqlCommand.Parameters.AddWithValue("@MaxDateOfBirth", filtering.MaxDateOfBirth);
            }

            stringBuilder.AppendLine($"ORDER BY {sorting.SortBy} {sorting.SortOrder}");

            stringBuilder.Append(" OFFSET " + offset + " ROWS FETCH NEXT ");
            stringBuilder.Append(paging.PageSize + " ROWS ONLY;");
